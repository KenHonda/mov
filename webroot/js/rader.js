...
function raderDraw(className, data_package, num, fColors){
	var rc = new html5jp.graph.radar(className);
  if( ! rc ) { return; }
	var interest = Number(data_package[num]['interest']);
	var kandou = Number(data_package[num]['kandou']);
	var cast = Number(data_package[num]['cast']);
	var worldview = Number(data_package[num]['worldview']);
	var trend = Number(data_package[num]['trend']);
	var kincho = Number(data_package[num]['kincho']);
	var items = [
...
window.onload = function() {
raderDraw("sample11", data_package, 0, "red");
>>>>>>> rebase前
};
...
addEvent(window,"load",function(){
  ...
  rc.draw(items, params);
=======
	raderDraw("sample14", data_package, 1, "#ff6600");
});
addEvent(window,"load",function(){
	raderDraw("sample15", data_package, 2, "navy");
});
addEvent(window,"load",function(){
	raderDraw("sample16", data_package, 3, "green");
});
addEvent(window,"load",function(){
	raderDraw("sample17", data_package, 4, "#ff66ff");
});
addEvent(window,"load",function(){
	raderDraw("sample18", data_package, 5, "#663300");
});
addEvent(window,"load",function(){
	raderDraw("sample19", data_package, 6, "#660099");
>>>>>>> rebase前
});
